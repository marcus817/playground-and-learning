...
function createWebPage (id, fetchTimeout) {
  var page = webpage.create();
  page.id = id;
  page.settings.resourceTimeout = fetchTimeout || 10000;
  ...
  page.onStrictRenderError = function() {};
...
var loop = function() {
  ...
  if (!page) page = createWebPage(line.id, line.fetchTimeout);
  else {
    page.id = line.id;
    page.settings.resourceTimeout = line.fetchTimeout;
  }
  ...
  page.onStrictRenderError = function() {
    if (!line.strictRender) return;
    fail();
  };

  var logAndFail = function(message) {
    page.log(message);
    fail();
  };
  ...
  page.open(line.url, function(requestStatus) {
    if (requestStatus !== 'success') return logAndFail({
      type: 'pageFetchError',
